...
export default function RolesPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data?.error || "Rollar yüklənmədi");
      }
  ...
  const createRole = async () => {
    ...
    if (!roleName) {
      setMsg({ type: "err", text: t.emptyRoleName });
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data?.error || "Rol yaratmaq alınmadı");
      }
  ...
  const remove = async (id: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data?.error || "Rol silmək alınmadı");
      }
        ...
        <div className="relative flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
              Rolları yaradın və sistemdə istifadəçilərin icazə strukturunu
              idarə edin.
            </p>
          </div>

          <div className="grid grid-cols-2 gap-3 sm:min-w-[300px]">
            <HeaderStat label="Cəmi" value={roles.length} />
            <HeaderStat label="Göstərilən" value={filteredRoles.length} />
          </div>
        </div>
      </section>
