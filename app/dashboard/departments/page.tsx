...
export default function DepartmentsPage() {
  ...
  const filteredDepartments = useMemo(() => {
    ...
    if (filterCompanyId) {
      data = data.filter((d) => String(d.company_id) === String(filterCompanyId));
    }

    return data;
  }, [departments, search, filterCompanyId]);
  ...
  const load = async () => {
    ...
    try {
      ...
      if (!cRes.ok) throw new Error(cData?.error || "Şirkətlər yüklənmədi");
      if (!dRes.ok) throw new Error(dData?.error || "Departamentlər yüklənmədi");
  ...
  const createDepartment = async () => {
    ...
    if (!departmentName) {
      setMsg({ type: "err", text: t.emptyDepartmentName });
      return;
    }

    if (!companyId) {
      setMsg({ type: "err", text: t.selectCompany });
      return;
    }
    ...
    try {
      const res = await fetch("/api/departments", {
        ...
        body: JSON.stringify({ name: departmentName, company_id: companyId }),
      });

      const data = await res.json();

      if (!res.ok) {
        throw new Error(data?.error || "Departament yaratmaq alınmadı");
      }
  ...
  const remove = async (id: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data?.error || "Departament silmək alınmadı");
      }
        ...
        <div className="relative flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
              Departamentləri yaradın, şirkətlərə bağlayın və struktur
              məlumatlarını idarə edin.
            </p>
          </div>

          <div className="grid grid-cols-2 gap-3 sm:min-w-[300px]">
            <HeaderStat label="Cəmi" value={departments.length} />
            <HeaderStat label="Göstərilən" value={filteredDepartments.length} />
          </div>
        </div>
      </section>
          ...
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div>
              ...
              <p className="mt-1 text-xs font-semibold text-slate-400">
                {filteredDepartments.length} nəticə
              </p>
            </div>
            ...
            <div className="flex w-full flex-col gap-2 sm:flex-row xl:w-auto">
              ...
              <div className="relative w-full xl:w-64">
                ...
                <select
                  ...
                  className="h-11 w-full rounded-2xl border border-slate-200 bg-slate-50 pl-11 pr-4 text-sm font-bold text-slate-700 outline-none transition focus:border-[#e42526] focus:bg-white focus:ring-4 focus:ring-[#e42526]/10"
                >
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-[1fr_1fr_auto]">
              ...
              <div className="relative">
                ...
                <select
                  ...
                  className="h-12 w-full rounded-2xl border border-slate-200 bg-white pl-11 pr-4 text-sm font-bold text-slate-700 outline-none transition focus:border-[#e42526] focus:ring-4 focus:ring-[#e42526]/10"
                >
                  <option value="">{t.selectCompany}</option>
                  {companies.map((c) => (
                    <option key={c.id} value={c.id}>
